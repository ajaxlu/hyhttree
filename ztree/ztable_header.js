...
var TableHelper2 = function () {
  if(this instanceof TableHelper2 ){
    ...
    this.tableHelper = {
      ...
      "freezecolsNum":1, //冻结列数，默认是0，可设置1，2,3,4（维度+序号）
      "freezhhead":1, //(0或者null表示不冻结,1表示冻结表头)
      "getAvailableScreenHeight":400, //设置表格高度，固定头部超出滚动 默认400
      "freezecolsWidth":$(window).outerWidth(true)-100, //冻结列表格宽度,默认最小值是0
      "pageinfo_front":false,//是否是前台分页
      "pageinfo_use":false,//是否使用pageinfo
      "pageinfo_callback":false,//是否使用回调
      "order_front":false,//是否是前台排序
    ...
    this.setting2 = {
      ...
      callback: {
        onCollapse: function (event, treeId, treeNode) {
          //$("#"+treeNode.tId.substring(5)+"_switch").click();
        },
    ...
    this.updateNode = function (list,type,up,startNum) {
      ...
      if(that.tableHelper.headList.length > 0) {
        ...
        //冻结列初始化构建的上层数据
        if(that.tableHelper.freezecolsNum>0&&that.tableHelper.freezecolsNum<4 && that.tableHelper.freezecolsWidth && that.tableHelper.freezecolsWidth > 0) {
    ...
    this.rendererView = function () {
      ...
      html +='</div>';
      //3、构建树形内容
      html += '<div id="headerAndData" class="headerAndData" style="overflow: hidden;">';
      ...
      if(that.tableHelper.pageinfo_use){
        if(that.tableHelper.userpagesize==1){
          //使用每页多少条显示功能
          html += '<div class="pagination_pagesize"><select id="pagesizeselt"  name="pagesizeselt"><option   value="5"> 5条/页 </option><option   value="10" selected = "selected"> 10条/页 </option><option   value="15"> 15条/页 </option><option   value="20"> 20条/页 </option><option   value="30"> 30条/页 </option></select></div>'
        }
        html +='<div class="pagination_box">';
        html +='  <div id="pagTabTool_'+that.tableHelper.id+'" class="pagTabTool sss"></div>';
        html +='  </div>';
      }
      html +='  </div>';
      //初始化页面
    ...
    this.initDiyDom2 = function (treeId, treeNode) {
      ...
      //冻结列数据截取
      _getLastHeadobj = that.getLastHeadAfterFilter();
    ...
    this.initPageInfo = function () {
      ...
      if(that.tableHelper.userpagesize==1){
        setTimeout(() => {
          ...
          that.callbackPageInfo(that.pageInfo.pageNum,that.pageInfo,"pageinfo",jqdiv);
        })
      }, 1);
      }
    ...
    this.initViewHead = function () {
      ...
      this.getTableHeadHtml = function (viewWidth,headList,type,tdWidth,headOverflow) {
        ...
        if(that.tableHelper.freezecolsNum>0&&that.tableHelper.freezecolsNum<4 && that.tableHelper.freezecolsWidth && that.tableHelper.freezecolsWidth > 0){
          $("#"+that.tableHelper.id).css({"width":(that.tableHelper.freezecolsWidth)+"px"});
        ...
        if(that.tableHelper.freezecolsNum > 0 && that.tableHelper.freezecolsNum < 4 && that.tableHelper.freezecolsWidth && that.tableHelper.freezecolsWidth > 0){
          html += '<div class="tableBoxDiv" style="overflow:auto;">'
        }else {
      ...
      this.getHeadTDHtml = function (data,type,tdWidth,isLast,headOverflow,rowIndex,colIndex) {
        ...
        if(that.tableHelper.temple=='diy'){
          var html = '<td class="td'+rowIndex+'_'+colIndex+'" rowspan="'+_rowspan+'" colspan="'+_colspan+'" style='+_style+'><div style="width:100%;height: 100%;padding:6px 0;">'+_value+'</div></td>';
        }else{
          if(_colspan>1){
            var html = '<td rowspan="'+_rowspan+'" colspan="'+_colspan+'" style='+_style+'><div style="width:100%;height: 100%;padding:6px 0;">'+_value+'</div></td>';
          }else{
            useWidth=useWidth-2;
            var html = '<td rowspan="'+_rowspan+'" colspan="'+_colspan+'" style='+_style+'><div style="width:'+useWidth+'px;height: 100%;padding:6px 0;">'+_value+'</div></td>';
          }
        }
        // var html = '<td rowspan="'+_rowspan+'" colspan="'+_colspan+'" style='+_style+'><div style="width:100%;height: 100%;padding:6px 0;">'+_value+'</div></td>';
        return html;
      ...
      if(that.tableHelper.headList.length>0 && that.tableHelper.showNodes && that.tableHelper.showNodes.length > 0) {
        ...
        rows.eq(0).before(headHtml);
        $("#"+that.tableHelper.id).append('<li ><div style="text-align: center;line-height: 70px;border:1px solid #e9e9e9;" >无符合条件数据</div></li>')
      }
    };
    //div容器的实际宽度
    ...
    this.freezeTableHeaderFn = function () {
      ...
      if(this.tableHelper.freezhhead == 1){
        var _getAvailableScreenHeight = this.tableHelper.getAvailableScreenHeight;//获取整个表的高度
        ...
        // if($(".ztreeBox").height()){
        //   if($(".headbox").height()){
        //     if($(".pagination_pagesize").height()){
        //       if($(".pagination_box").height()){
        //          _getAvailableScreenHeight = _getAvailableScreenHeight - $(".ztreeBox").height() - $(".headbox").height() - $(".pagination_pagesize").height() - $(".pagination_box").height();
        //       }else {
        //          _getAvailableScreenHeight = _getAvailableScreenHeight - $(".ztreeBox").height() - $(".headbox").height() - $(".pagination_pagesize").height();
        //       }
        //     }else {
        //        _getAvailableScreenHeight = _getAvailableScreenHeight - $(".ztreeBox").height() - $(".headbox").height();
        //     }
        //   }else {
        //     _getAvailableScreenHeight = _getAvailableScreenHeight - $(".ztreeBox").height();
        //   }
        // }
        ...
        //底层数据超出产生滚动
        if(chazhiheight>0){
          $(".ztree.ztreetable.noFreezecolsDiv").css({"overflow":"auto","height":_getAvailableScreenHeight+"px"});
          $(".ztree.ztreetable.freezecolsDiv").css({"overflow":"auto","height":_getAvailableScreenHeight+"px"});  //监听数据超出产生滚动，表头也产生固定条，保证宽度一致
          //判断数据是否超出，让维度减去滚动条高度
          var chazhiwidth = $(".ztree.ztreetable.noFreezecolsDiv li").width() - $(".ztree.ztreetable.noFreezecolsDiv").width();
          if(chazhiwidth>0){
            $(".ztree.ztreetable.freezecolsDiv").height(_getAvailableScreenHeight - 17);
          }
        }

        //if($(".ztree.ztreetable.freezecolsDiv").height() > _getAvailableScreenHeight){
        //数据滚动条 监听左边维度滚动
        // $(".ztree.ztreetable.freezecolsDiv").scrollTop(10);
        // if($(".ztree.ztreetable.freezecolsDiv").scrollTop()>0 ){
        //   $(".headbox").addClass("addOverFlow");
        //   var top = $(".headbox").scrollTop();
        //   $(".headbox").on('scroll.unable',function (e) {
        //     $(".headbox").scrollTop(top);
        //   });
        //   //减去滚动条高度
        //   $(".ztree.ztreetable.freezecolsDiv").height($(".ztree.ztreetable.freezecolsDiv").height() - 17);
        //
        // }else{
        //   $(".headbox").removeClass("addOverFlow");
        // }
        //$(".ztree.ztreetable.freezecolsDiv").scrollTop(0);
        // }
      }
    };
    //页面加载完渲染
    this.rendererViewAfter = function (up) {
      ...
      //固定表头
      $(".ztreetable_section .ztreeBox").css("positon","relative");
      $(".headbox").css({"positon":"absolute",'left':"0",'top':"0","z-index":"99"});
      ...
      if(that.tableHelper.freezecolsNum > 0 && that.tableHelper.freezecolsNum < 4 && that.tableHelper.freezecolsWidth && that.tableHelper.freezecolsWidth > 0){
        //让固定列跟着数据一起滚动
        $(".ztree.ztreetable.noFreezecolsDiv").on('scroll',function(){
          $(".ztree.ztreetable.freezecolsDiv").scrollTop($(".ztree.ztreetable.noFreezecolsDiv").scrollTop());
        });
        //滚动固定列层禁用（单独滚动带不起底层数据滚动）
        $(".ztree.ztreetable.freezecolsDiv").on('scroll.unable',function () {
          $(".ztree.ztreetable.freezecolsDiv").scrollTop($(".ztree.ztreetable.noFreezecolsDiv").scrollTop());
        });
        ...
        if(that.tableHelper.temple == "simple"){
          //td平均宽度
          $(".ztree.ztreetable.freezecolsDiv").css({"width":that.getTDWidth()*_getLastHeadAfterFilterList.length,"top":headboxHeight+"px","left":"0"})
        }else {
          //td设置的宽度minWidth
          $(".ztree.ztreetable.freezecolsDiv").css({"width":_allAddWidth,"top":headboxHeight+"px","left":"0"})
        }
    ...
    this.getContentHtml = function (data,rlist,dimensionality,type,width,level) {
      ...
      if(data){
        ...
        for( ;i<rlist.length;i++){
          ...
          html += '<div class="'+_class+'" style="'+_style+'" title="'+_value+'"><span '+ifHasIcon._attribute_font+' class="' + ifHasIcon._class_font +'">'+_value+'</span>'+ifHasIcon._value_icon_html+'</div>';
        }
      }
      return html;
    };
