var TreeHelper = function () {
  if (this instanceof TreeHelper) {
    ...
    this.isCheck = false;
    this.isRMenu = false;
    this.isExpandAll = false; //是否默认展开子节点
    this.limitZtreeHeight = null; //限制组件的高度
    this.setLimitZtreeHeight = function(value){ //限制组件的高度
      if(value){
        that.limitZtreeHeight = value;
      }
    }; 
    this.setting = {
      view: {
    ...
    this.initView = function () {
      ...
      console.log($("#"+that.id).height());
      // 限制树的高度
      if (that.limitZtreeHeight != null) {
        that.limitZtreeHeight>100?that.limitZtreeHeight=that.limitZtreeHeight-100:that.limitZtreeHeight;
        html += '    <ul style="height:'+that.limitZtreeHeight+'px;" id="' + this.id + '" class="ztree '+that.sysMenuBtn  +'"  ></ul>';
      }else{
    ...
    this.init = function () {
      ...
      that.zTree = $.fn.zTree.getZTreeObj(that.id); //初始化ztree树
      // 是否默认展开所有节点
      if(that.isExpandAll){
        that.zTree.expandAll(true);
      }else{
        that.zTree.expandAll(false);
      }
    ...
    this.createSimpleMenu = function (viewId, zNodes, zcfun) {
      ...
      this.viewId = viewId;
      this.sysMenuBtn = "createSimpleMenuBtn";      
    ...
    this.createOperateMenu = function (viewId, zNodes, zcfun, ycqfun, ychfun) {
      ...
      this.viewId = viewId;
      this.sysMenuBtn = "createOperateMenuBtn";
      this.zNodes = zNodes;
      this.zcfun = zcfun;
      this.ycqfun = ycqfun;
      this.ychfun = ychfun;
    ...
    this.createDIYMenu = function (viewId, zNodes,diyMenu, zcfun, ycqfun, ychfun) {
      this.menuType = "diy";
      this.sysMenuBtn = "createDIYMenuBtn";
      this.viewId = viewId;
      this.zNodes = zNodes;
      this.zcfun = zcfun;
      this.ycqfun = ycqfun;
    ...
    this.showRMenu = function (type, x, y) {
      ...
      $(that.rMenuKey).css({
        "top": y - $("#" + that.id).offset().top + 40 + "px",
